...
class Admin extends CI_Controller {
	...
	public function clients(){
		$page_data=array(
			...
			'totalCollectors' => $this->qm->collectors('countCollectors'),//count collectors;
			'collectorsQuery' => $this->qm->collectors('queryCollectors'),
			);
		$this->load->view('index', $page_data,'refresh');//load index
	}
	...
	public function disposes_crud($p1="",$p2=""){
		// Fallback session check in case constructor is bypassed
		if ($this->session->userdata('logged_in') == FALSE) {
			redirect(base_url()."login", 'refresh');
			return;
		}
		...
		if($p1=="approve"){
			// Approve should set both payment_status and transaction_status
			$approve=$this->adm->disposes_crud("approve",$p2);
			if($approve){
				$this->session->set_flashdata('approve_message' , 'Dispose id <b>#'.$p2.'</b> was approved successfully!');
			}
			redirect ('admin/disposes','refresh');
			return;
		}
	...
	public function clients_crud($p1="",$p2=""){
		...
		if($p1=="add_collector"){
			...
			array(
				'field' => 'collector_password2',
				'label' => 'Retype password',
				'rules' => 'required|matches[collector_password]',
				'errors' => array(
					'required' => 'The %s field is required!',
					'matches' => 'The %s field should match password field!',
					)
			...
			if($this->form_validation->run() === true) {
				...
				$reg = $this->lm->register_collector();
	
				if($reg === true) {
					$validator['success'] = true;
					$validator['messages'] = "Collector Added Successfully!";
				}	
				else {
					$validator['success'] = false;
					$validator['messages'] = "There was an error while posting your data.";
		...
		if($p1=="delete"){
			...
			if($delete){		 
			$this->session->set_flashdata('client_message' , 'User Deleted Successfully!');
