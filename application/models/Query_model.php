...
class Query_model extends CI_Model {
    // Hide pin by transaction_code (set hidden flag)
    public function hide_pin_by_code($transaction_code) {
        if (!$transaction_code) return false;
        $this->db->where('transaction_code', $transaction_code);
        return $this->db->update('transaction', ['hidden' => 1]);
    }
    ...
    //collectors queries
    public function collectors($param1=null,$param2=null){
        $this->check_session();
        if($param1=="countCollectors"){
            $where=array("role"=>'collector',"user_deleted"=>0);
            $this->db->select('*');
            $this->db->from('login');
            $this->db->where($where);
            return  $this->db->count_all_results();
        }
        if($param1=="queryCollectors"){
            $where=array("role"=>'collector',"user_deleted"=>0);
            $this->db->select('*');
            $this->db->from('login');
            $this->db->where($where);
    ...
    public function disposes($param1=null,$param2=null){
        // Automatically hide pin when transaction is collected
        if ($param1 === 'autoHideCollectedPins') {
            $this->db->where('transaction_status', 2); // 2 = Collected
            $this->db->where('IFNULL(hidden,0) !=', 1);
            $pins = $this->db->get('transaction')->result_array();
            foreach ($pins as $pin) {
                $this->db->where('transaction_id', $pin['transaction_id']);
                $this->db->update('transaction', ['hidden' => 1]);
            }
            return true;
        }
        ...
        if($param1=="adminQueryDisposes"){
            $this->db->select('*');
            $this->db->from('transaction');
            $this->db->where('IFNULL(hidden,0) !=', 1); // Only applies to transaction table (map)
            $this->db->order_by('transaction_id','desc');
            return $this->db->get()->result_array();
        }
    ...
    public function dueDate($collectDate){
        $this->check_session();
        // If $collectDate is empty or not numeric, try to convert it
        if (empty($collectDate)) {
            return 0;
        }
        if (!is_numeric($collectDate)) {
            $timestamp = strtotime($collectDate);
            if ($timestamp === false) {
                return 0;
            }
        } else {
            $timestamp = (int)$collectDate;
        }
    ...
    function sendNotification($code){
        ...
        $message='Hello '.$name. '? Your dispose request has been recieved successfully. Your dispose ID is '.$code.'. Please pay : '. $this->formatMoney($transaction_total,true). ' before '.date("m/d/Y",$collection_date).' to approve your disposes.';
        //send message
        $curl = curl_init();
