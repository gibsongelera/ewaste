...
class Login_model extends CI_Model {
	...
	public function register(){
		...
		$date_reg=date('dMY');
		$role = $this->input->post('role') ? $this->input->post('role') : 'client';
	...
	public function register_collector(){
		...
		$email = $this->input->post('collector_email');
		$name = $this->input->post('collector_name');
		$password=password_hash($this->input->post('collector_password'),PASSWORD_DEFAULT);
		$date_reg=date('dMY');

		//collector login array
		$add_collector_login_data = array(
			'name' => $name,
			'email' => $email,
			'password' => $password,
			'role'=>'collector',
		...
		//post login
		 $added_collector_login_data=$this->db->insert('login',$add_collector_login_data);

		//get last id
		 $last_insert = $this->db->insert_id();


		//collector profile array
		$add_collector_profile_data = array(
			'date_registered'=>strtotime($date_reg),
			'login_id'=>$last_insert,
		);

		//post login
		 $added_collector_profile_data=$this->db->insert('profiles',$add_collector_profile_data);

		 //logs
		 $log= array(
		 	'message' => 'New collector registered',
		 	'user_id' => $last_insert,
		 	'trigger_date'=>strtotime('now')
		 );
		 //inset logs table
		 ...
		 //if success
		 if($added_collector_login_data && $added_collector_profile_data && $insert_log){
			return true;
		}else{
			return false;
		}
}//end register_collector function
