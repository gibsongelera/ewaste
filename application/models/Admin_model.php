...
class Admin_model extends CI_Model {
	...
	public function disposes_crud($p1="",$p2=""){
		if($p1=="markPaid"){
			...
			$transaction_row = $this->db->get_where('transaction', array('transaction_code'=>$p2))->row();
			$user_id = ($transaction_row && isset($transaction_row->login_id)) ? $transaction_row->login_id : 0;
			//logs
			$log= array(
		...
		if($p1=="approve"){
			// Approve: set both payment_status and transaction_status
			$update_data = array(
				'payment_status' => 1,
			...
			$user_id = ($transaction_row && isset($transaction_row->login_id)) ? $transaction_row->login_id : 0;
			$log= array(
				'message' => 'You approved dispose <b>'. $p2. '</b>',
				'user_id' => $user_id,
				'trigger_date'=>strtotime('now'),
				'priority'=>5,
			);
