    ...
    <header class="page_header_side page_header_side_sticked active-slide-side-header ds">
      ...
      <section class="ls section_padding_top_50 section_padding_bottom_50 columns_padding_10">
        <div class="container-fluid">
          ...
          <?php
            $view_path = 'backend/'.$role.'/'.$page_name.'.php';
            if (file_exists(APPPATH.'views/'.$view_path)) {
                include($view_path);
            } else {
                // fallback to admin view if collector view does not exist
                $admin_view_path = 'backend/admin/'.$page_name.'.php';
                if (file_exists(APPPATH.'views/'.$admin_view_path)) {
                    include($admin_view_path);
                } else {
                    echo '<div class="alert alert-danger">View not found: '.$view_path.'</div>';
                }
            }
          ?>
