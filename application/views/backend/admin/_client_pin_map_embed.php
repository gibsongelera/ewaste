    ...
    .then(pins => {
        ...
        pins.forEach(pin => {
            if (pin.lat && pin.lng) {
                ...
                // Add info window with details and delete button
                const infoWindow = new google.maps.InfoWindow({
                    content: `
                        <div style="padding:10px;">
                            ...
                            <button id="delete-pin-btn-${pin.transaction_code}" style="margin-top:10px; background-color:#d9534f; color:white; border:none; border-radius:4px; padding:5px 12px; cursor:pointer; transition:background 0.2s;" class="btn btn-danger btn-sm" onmouseover="this.style.backgroundColor='#c9302c'" onmouseout="this.style.backgroundColor='#d9534f'">Delete Pin</button>
                        </div>
                    `
                });
                marker.addListener('click', () => {
                    ...
                    setTimeout(() => {
                        const btn = document.getElementById(`delete-pin-btn-${pin.transaction_code}`);
                        if (btn) {
                            btn.onclick = function(e) {
                                e.stopPropagation();
                                deletePin(pin.transaction_code);
                            };
                        }
                        // Prevent navigation when clicking delete button
                        const infoDiv = btn && btn.parentElement;
                        if (infoDiv) {
...
// Manual delete pin function (global scope)
function deletePin(transaction_code) {
    if (!confirm('Are you sure you want to delete this pin?')) return;
    fetch('<?php echo base_url('admin/delete_pin'); ?>', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
        },
        body: 'transaction_code=' + encodeURIComponent(transaction_code)
    })
